...
export function DownloadButton(props: DownloadButtonProps) {
  ...
  const architecture =
    props.destination === "download-page"
      ? "arm64"
  ...
  const classes = [
    ...
    compact ? "compact" : "",
    inverse ? "inverse" : "",
    props.destination !== "download-page" && architecture === "armv7"
      ? "secondary"
      : "",
  ]
    .filter(Boolean)
  ...
  if (!release.downloadUrl) {
    return (
      <button
        ...
        type="button"
        disabled
        aria-label={`${release.architecture} APK download is not configured yet`}
      >
        <DownloadGlyph />
        <span>{label || "Download unavailable"}</span>
      </button>
    );
  }
  ...
  return (
    <m.a
      ...
      aria-label={`Download Yaari24 version ${APK.version} ${release.architecture} Android APK`}
    >
      <DownloadGlyph />
...
export function PlayStoreBadge({ inverse = false }: { inverse?: boolean }) {
  return (
    <div
      className={"play-badge " + (inverse ? "inverse" : "")}
      aria-label="Google Play release coming soon"
      aria-disabled="true"
    >
      <svg aria-hidden="true" viewBox="0 0 24 24">
        <path d="m4 3 11 9L4 21V3Z" fill="#34A853" />
        <path d="m4 3 13 7-2 2L4 3Z" fill="#4285F4" />
        <path d="m4 21 13-7-2-2L4 21Z" fill="#FBBC04" />
        <path d="m17 10 3 2-3 2-2-2 2-2Z" fill="#EA4335" />
      </svg>
      <span>

...
export function DownloadCta({ compact = false }: { compact?: boolean }) {
  return <section className={'download-cta ' + (compact ? 'compact' : '')}>
    ...
    <Reveal className="cta-copy" delay={.06}><span className="eyebrow">Ready when you are</span><h2>Bas tap karo.<br /><em>Yaari starts here.</em></h2><p>Download the official 64-bit Android APK for most phones. Older-phone download is available on the Download page.</p><div className="cta-actions"><DownloadButton inverse /><PlayStoreBadge inverse /></div><div className="cta-meta"><span>Android</span><i /><span>v{APK.version}</span><i /><span>{APK.arm64.size}</span></div></Reveal>
  </section>;
}

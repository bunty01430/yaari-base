...
export default function SafetyPage() {
  return <PageTransition>
    ...
    <section className="section safety-main"><div className="container safety-main-grid"><Reveal><span className="eyebrow">Safety in the conversation</span><h2>See it. Act on it.<br /><em>Keep moving.</em></h2><p>The controls live where you need them. Report a specific message, block unwanted contact, or let room moderators step in.</p></Reveal><SafetyFlow /></div></section>
    <section className="section"><div className="container"><div className="safety-principles">{principles.map((item, index) => <Reveal className="principle" key={item.title} delay={index * .06}><span className="principle-number">0{index + 1}</span><img src={item.icon} alt="" width="76" height="76" loading="lazy" /><h2>{item.title}</h2><p>{item.copy}</p></Reveal>)}</div></div></section>
    <section className="community-rules"><div className="container rules-grid"><Reveal><span className="eyebrow">The simple rule</span><h2>Bring the energy.<br /><em>Respect the room.</em></h2></Reveal><Reveal delay={.1}><p>Be yourself without making someone else feel smaller. No harassment, hate, impersonation, unwanted sexual content, or behavior that puts people at risk.</p><p>Reports are reviewed in context. Serious or repeated violations can lead to room restrictions or account action.</p></Reveal></div></section>
